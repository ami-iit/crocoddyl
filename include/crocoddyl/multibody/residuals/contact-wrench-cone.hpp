      ...
      for (typename ContactModelMultiple::ContactDataContainer::iterator it = d1->contacts->contacts.begin();
        ...
        if (it->second->frame == id) {
          ...
          if (d3d != NULL) {
            found_contact = true;
            contact = it->second;
            throw_pretty("Domain error: there isn't defined at least a 6d contact (just 3d) for " + frame_name);
            break;
          }
          ...
          if (d5d != NULL) {
            found_contact = true;
            contact = it->second;
            throw_pretty("Domain error: there isn't defined at least a 6d contact (just 5d) for " + frame_name);
            break;
          }
