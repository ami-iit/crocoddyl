...
DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::DifferentialActionModelContactFwdDynamicsSEATpl(
...
template <typename Scalar>
void DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::calc(
  ...
  d->multibody.actuation->tau[r_sus] -= (10*q[r_sus] + 10*v[r_sus]);
  d->multibody.actuation->tau[l_sus] -= (10*q[l_sus] + 10*v[l_sus]);
...
template <typename Scalar>
void DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::calcDiff(
    const boost::shared_ptr<DifferentialActionDataAbstract>& data, const Eigen::Ref<const VectorXs>& x,
    const Eigen::Ref<const VectorXs>& u) {
  if (static_cast<std::size_t>(x.size()) != state_->get_nx()) {
    throw_pretty("Invalid argument: "
...
DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::createData() {
  return boost::allocate_shared<Data>(Eigen::aligned_allocator<Data>(), this);
}

template <typename Scalar>
void DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::quasiStatic(
...
template <typename Scalar>
bool DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::checkData(
...
template <typename Scalar>
void DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::print(std::ostream& os) const {
  os << "DifferentialActionModelContactFwdDynamicsSEA {nx=" << state_->get_nx() << ", ndx=" << state_->get_ndx()
     << ", nu=" << nu_ << ", nc=" << contacts_->get_nc() << "}";
}

template <typename Scalar>
pinocchio::ModelTpl<Scalar>& DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::get_pinocchio() const {
  return pinocchio_;
}

template <typename Scalar>
const boost::shared_ptr<ActuationModelAbstractTpl<Scalar> >&
DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::get_actuation() const {
  return actuation_;
}

template <typename Scalar>
const boost::shared_ptr<ContactModelMultipleTpl<Scalar> >&
DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::get_contacts() const {
  return contacts_;
}

template <typename Scalar>
const boost::shared_ptr<CostModelSumTpl<Scalar> >& DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::get_costs()
    const {
  return costs_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::VectorXs& DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::get_armature()
    const {
  return armature_;
}

template <typename Scalar>
const Scalar DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::get_damping_factor() const {
  return JMinvJt_damping_;
}

template <typename Scalar>
void DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::set_armature(const VectorXs& armature) {
...
template <typename Scalar>
void DifferentialActionModelContactFwdDynamicsSEATpl<Scalar>::set_damping_factor(const Scalar damping) {
  if (damping < 0.) {
    throw_pretty("Invalid argument: "
