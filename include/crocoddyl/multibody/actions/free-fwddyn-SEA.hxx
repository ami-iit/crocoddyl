...
DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::DifferentialActionModelFreeFwdDynamicsSEATpl(
...
template <typename Scalar>
void DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::calc(
...
template <typename Scalar>
void DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::calcDiff(
    const boost::shared_ptr<DifferentialActionDataAbstract>& data, const Eigen::Ref<const VectorXs>& x,
    const Eigen::Ref<const VectorXs>& u) {
  if (static_cast<std::size_t>(x.size()) != state_->get_nx()) {
    throw_pretty("Invalid argument: "
...
DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::createData() {
  return boost::allocate_shared<Data>(Eigen::aligned_allocator<Data>(), this);
}

template <typename Scalar>
bool DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::checkData(
...
template <typename Scalar>
void DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::quasiStatic(
...
template <typename Scalar>
void DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::print(std::ostream& os) const {
  os << "DifferentialActionModelFreeFwdDynamics {nx=" << state_->get_nx() << ", ndx=" << state_->get_ndx()
     << ", nu=" << nu_ << "}";
}

template <typename Scalar>
pinocchio::ModelTpl<Scalar>& DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::get_pinocchio() const {
  return pinocchio_;
}

template <typename Scalar>
const boost::shared_ptr<ActuationModelAbstractTpl<Scalar> >&
DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::get_actuation() const {
  return actuation_;
}

template <typename Scalar>
const boost::shared_ptr<CostModelSumTpl<Scalar> >& DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::get_costs()
    const {
  return costs_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::VectorXs& DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::get_armature() const {
  return armature_;
}

template <typename Scalar>
void DifferentialActionModelFreeFwdDynamicsSEATpl<Scalar>::set_armature(const VectorXs& armature) {
