...
FrictionCopConeTpl<Scalar>::FrictionCopConeTpl()
    : nf_(4),
      A_(nf_ + 5, 5),
      ub_(nf_ + 5),
      lb_(nf_ + 5),
...
template <typename Scalar>
FrictionCopConeTpl<Scalar>::FrictionCopConeTpl(const Matrix3s& R, const Scalar mu, const Vector2s& box, const std::size_t nf,
  ...
  A_ = MatrixX6s::Zero(nf_ + 5, 5);
  ub_ = VectorXs::Zero(nf_ + 5);
  lb_ = VectorXs::Zero(nf_ + 5);
...
template <typename Scalar>
FrictionCopConeTpl<Scalar>::FrictionCopConeTpl(const Matrix3s& R, const Scalar mu, const Vector2s& box, std::size_t nf,
...
template <typename Scalar>
FrictionCopConeTpl<Scalar>::FrictionCopConeTpl(const FrictionCopConeTpl<Scalar>& cone)
    : nf_(cone.get_nf()),
      A_(cone.get_A()),
      ub_(cone.get_ub()),
      lb_(cone.get_lb()),
...
template <typename Scalar>
void FrictionCopConeTpl<Scalar>::update() {
  ...
  //  0  0 -L  0  1  0;
  //  0  0 -L  0 -1  0]
  A_.row(nf_ + 1) << -W * R_.col(2).transpose(), (R_.col(0).transpose() ).head(2);
  A_.row(nf_ + 2) << -W * R_.col(2).transpose(), (-R_.col(0).transpose() ).head(2);
  A_.row(nf_ + 3) << -L * R_.col(2).transpose(), (R_.col(1).transpose() ).head(2);
  A_.row(nf_ + 4) << -L * R_.col(2).transpose(), (-R_.col(1).transpose() ).head(2);
}

template <typename Scalar>
void FrictionCopConeTpl<Scalar>::update(const Matrix3s& R, const Scalar mu, const Vector2s& box, const Scalar min_nforce,
...
template <typename Scalar>
const typename MathBaseTpl<Scalar>::MatrixX6s& FrictionCopConeTpl<Scalar>::get_A() const {
  return A_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::VectorXs& FrictionCopConeTpl<Scalar>::get_ub() const {
  return ub_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::VectorXs& FrictionCopConeTpl<Scalar>::get_lb() const {
  return lb_;
}

template <typename Scalar>
std::size_t FrictionCopConeTpl<Scalar>::get_nf() const {
  return nf_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::Matrix3s& FrictionCopConeTpl<Scalar>::get_R() const {
  return R_;
}

template <typename Scalar>
const typename MathBaseTpl<Scalar>::Vector2s& FrictionCopConeTpl<Scalar>::get_box() const {
  return box_;
}

template <typename Scalar>
const Scalar FrictionCopConeTpl<Scalar>::get_mu() const {
  return mu_;
}

template <typename Scalar>
bool FrictionCopConeTpl<Scalar>::get_inner_appr() const {
  return inner_appr_;
}

template <typename Scalar>
const Scalar FrictionCopConeTpl<Scalar>::get_min_nforce() const {
  return min_nforce_;
}

template <typename Scalar>
const Scalar FrictionCopConeTpl<Scalar>::get_max_nforce() const {
  return max_nforce_;
}

template <typename Scalar>
void FrictionCopConeTpl<Scalar>::set_R(const Matrix3s& R) {
  R_ = R;
}

template <typename Scalar>
void FrictionCopConeTpl<Scalar>::set_box(const Vector2s& box) {
  box_ = box;
  if (box_(0) < Scalar(0.)) {
...
template <typename Scalar>
void FrictionCopConeTpl<Scalar>::set_mu(const Scalar mu) {
  if (mu < Scalar(0.)) {
    mu_ = Scalar(1.);
    std::cerr << "Warning: mu has to be a positive value, set to 1." << std::endl;
  }
...
template <typename Scalar>
void FrictionCopConeTpl<Scalar>::set_inner_appr(const bool inner_appr) {
  inner_appr_ = inner_appr;
}

template <typename Scalar>
void FrictionCopConeTpl<Scalar>::set_min_nforce(const Scalar min_nforce) {
  if (min_nforce < Scalar(0.)) {
    min_nforce_ = Scalar(0.);
    std::cerr << "Warning: min_nforce has to be a positive value, set to 0" << std::endl;
  }
...
template <typename Scalar>
void FrictionCopConeTpl<Scalar>::set_max_nforce(const Scalar max_nforce) {
  if (max_nforce < Scalar(0.)) {
    max_nforce_ = std::numeric_limits<Scalar>::infinity();
    std::cerr << "Warning: max_nforce has to be a positive value, set to infinity value" << std::endl;
  }
...
template <typename Scalar>
FrictionCopConeTpl<Scalar>& FrictionCopConeTpl<Scalar>::operator=(const FrictionCopConeTpl<Scalar>& other) {
  if (this != &other) {
    nf_ = other.get_nf();
    A_ = other.get_A();
    ub_ = other.get_ub();
...
template <typename Scalar>
std::ostream& operator<<(std::ostream& os, const FrictionCopConeTpl<Scalar>& X) {
